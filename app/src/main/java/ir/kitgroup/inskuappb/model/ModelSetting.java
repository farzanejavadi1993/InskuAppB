package ir.kitgroup.inskuappb.model;

import androidx.annotation.Keep;

import com.google.gson.annotations.Expose;
import com.google.gson.annotations.SerializedName;
@Keep
public class ModelSetting {
    @SerializedName("PA")
    @Expose
    private String pa;
    @SerializedName("SP")
    @Expose
    private String sp;
    @SerializedName("OP")
    @Expose
    private String op;
    @SerializedName("DS")
    @Expose
    private String ds;
    @SerializedName("BS")
    @Expose
    private String bs;
    @SerializedName("PB")
    @Expose
    private String pb;
    @SerializedName("CE")
    @Expose
    private String ce;
    @SerializedName("CP")
    @Expose
    private String cp;
    @SerializedName("CA")
    @Expose
    private String ca;
    @SerializedName("CD")
    @Expose
    private String cd;
    @SerializedName("SU")
    @Expose
    private String su;
    @SerializedName("FR")
    @Expose
    private String fr;
    @SerializedName("CM")
    @Expose
    private String cm;
    @SerializedName("SH")
    @Expose
    private String sh;
    @SerializedName("US")
    @Expose
    private String us;
    @SerializedName("USD")
    @Expose
    private String usd;
    @SerializedName("USC")
    @Expose
    private String usc;
    @SerializedName("CLUB")
    @Expose
    private String club;
    @SerializedName("TAU")
    @Expose
    private String tau;
    @SerializedName("DC")
    @Expose
    private String dc;
    @SerializedName("RC")
    @Expose
    private String rc;
    @SerializedName("DV")
    @Expose
    private String dv;
    @SerializedName("RV")
    @Expose
    private String rv;
    @SerializedName("DA")
    @Expose
    private String da;
    @SerializedName("RA")
    @Expose
    private String ra;
    @SerializedName("LSP")
    @Expose
    private String lsp;
    @SerializedName("ST")
    @Expose
    private String st;
    @SerializedName("GC")
    @Expose
    private String gc;
    @SerializedName("G")
    @Expose
    private String g;
    @SerializedName("GS")
    @Expose
    private String gs;
    @SerializedName("EA")
    @Expose
    private String ea;
    @SerializedName("OO")
    @Expose
    private String oo;
    @SerializedName("AAO")
    @Expose
    private String aao;
    @SerializedName("ACO")
    @Expose
    private String aco;
    @SerializedName("CRCHQ")
    @Expose
    private String crchq;
    @SerializedName("CPCHQ")
    @Expose
    private String cpchq;
    @SerializedName("CACCB")
    @Expose
    private String caccb;
    @SerializedName("DUPO")
    @Expose
    private String dupo;
    @SerializedName("TRC")
    @Expose
    private String trc;
    @SerializedName("LD")
    @Expose
    private String ld;
    @SerializedName("CMD")
    @Expose
    private String cmd;
    @SerializedName("STI")
    @Expose
    private String sti;
    @SerializedName("ETI")
    @Expose
    private String eti;
    @SerializedName("CHPR")
    @Expose
    private String chpr;
    @SerializedName("OFA")
    @Expose
    private String ofa;

    public String getPa() {
        return pa;
    }

    public void setPa(String pa) {
        this.pa = pa;
    }

    public String getSp() {
        return sp;
    }

    public void setSp(String sp) {
        this.sp = sp;
    }

    public String getOp() {
        return op;
    }

    public void setOp(String op) {
        this.op = op;
    }

    public String getDs() {
        return ds;
    }

    public void setDs(String ds) {
        this.ds = ds;
    }

    public String getBs() {
        return bs;
    }

    public void setBs(String bs) {
        this.bs = bs;
    }

    public String getPb() {
        return pb;
    }

    public void setPb(String pb) {
        this.pb = pb;
    }

    public String getCe() {
        return ce;
    }

    public void setCe(String ce) {
        this.ce = ce;
    }

    public String getCp() {
        return cp;
    }

    public void setCp(String cp) {
        this.cp = cp;
    }

    public String getCa() {
        return ca;
    }

    public void setCa(String ca) {
        this.ca = ca;
    }

    public String getCd() {
        return cd;
    }

    public void setCd(String cd) {
        this.cd = cd;
    }

    public String getSu() {
        return su;
    }

    public void setSu(String su) {
        this.su = su;
    }

    public String getFr() {
        return fr;
    }

    public void setFr(String fr) {
        this.fr = fr;
    }

    public String getCm() {
        return cm;
    }

    public void setCm(String cm) {
        this.cm = cm;
    }

    public String getSh() {
        return sh;
    }

    public void setSh(String sh) {
        this.sh = sh;
    }

    public String getUs() {
        return us;
    }

    public void setUs(String us) {
        this.us = us;
    }

    public String getUsd() {
        return usd;
    }

    public void setUsd(String usd) {
        this.usd = usd;
    }

    public String getUsc() {
        return usc;
    }

    public void setUsc(String usc) {
        this.usc = usc;
    }

    public String getClub() {
        return club;
    }

    public void setClub(String club) {
        this.club = club;
    }

    public String getTau() {
        return tau;
    }

    public void setTau(String tau) {
        this.tau = tau;
    }

    public String getDc() {
        return dc;
    }

    public void setDc(String dc) {
        this.dc = dc;
    }

    public String getRc() {
        return rc;
    }

    public void setRc(String rc) {
        this.rc = rc;
    }

    public String getDv() {
        return dv;
    }

    public void setDv(String dv) {
        this.dv = dv;
    }

    public String getRv() {
        return rv;
    }

    public void setRv(String rv) {
        this.rv = rv;
    }

    public String getDa() {
        return da;
    }

    public void setDa(String da) {
        this.da = da;
    }

    public String getRa() {
        return ra;
    }

    public void setRa(String ra) {
        this.ra = ra;
    }

    public String getLsp() {
        return lsp;
    }

    public void setLsp(String lsp) {
        this.lsp = lsp;
    }

    public String getSt() {
        return st;
    }

    public void setSt(String st) {
        this.st = st;
    }

    public String getGc() {
        return gc;
    }

    public void setGc(String gc) {
        this.gc = gc;
    }

    public String getG() {
        return g;
    }

    public void setG(String g) {
        this.g = g;
    }

    public String getGs() {
        return gs;
    }

    public void setGs(String gs) {
        this.gs = gs;
    }

    public String getEa() {
        return ea;
    }

    public void setEa(String ea) {
        this.ea = ea;
    }

    public String getOo() {
        return oo;
    }

    public void setOo(String oo) {
        this.oo = oo;
    }

    public String getAao() {
        return aao;
    }

    public void setAao(String aao) {
        this.aao = aao;
    }

    public String getAco() {
        return aco;
    }

    public void setAco(String aco) {
        this.aco = aco;
    }

    public String getCrchq() {
        return crchq;
    }

    public void setCrchq(String crchq) {
        this.crchq = crchq;
    }

    public String getCpchq() {
        return cpchq;
    }

    public void setCpchq(String cpchq) {
        this.cpchq = cpchq;
    }

    public String getCaccb() {
        return caccb;
    }

    public void setCaccb(String caccb) {
        this.caccb = caccb;
    }

    public String getDupo() {
        return dupo;
    }

    public void setDupo(String dupo) {
        this.dupo = dupo;
    }

    public String getTrc() {
        return trc;
    }

    public void setTrc(String trc) {
        this.trc = trc;
    }

    public String getLd() {
        return ld;
    }

    public void setLd(String ld) {
        this.ld = ld;
    }

    public String getCmd() {
        return cmd;
    }

    public void setCmd(String cmd) {
        this.cmd = cmd;
    }

    public String getSti() {
        return sti;
    }

    public void setSti(String sti) {
        this.sti = sti;
    }

    public String getEti() {
        return eti;
    }

    public void setEti(String eti) {
        this.eti = eti;
    }

    public String getChpr() {
        return chpr;
    }

    public void setChpr(String chpr) {
        this.chpr = chpr;
    }

    public String getOfa() {
        return ofa;
    }

    public void setOfa(String ofa) {
        this.ofa = ofa;
    }

}
